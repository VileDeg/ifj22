...
/////////////////////////////////////////////////////////////////////////
	...
	#define KEYWORD_IS(_kw)    (TOKEN_IS(keyword) && pd->token.keyword == keyword_##_kw)
			 ...
			 E.g. : 'if', 'else', 'EOF' etc. */
		#define CHECK_TOKEN(_postfix, ...) SYNTAX(TOKEN_IS(_postfix##__VA_ARGS__))
		#define CHECK_KEYWORD(_kw, ...)    SYNTAX(KEYWORD_IS(_kw##__VA_ARGS__)	 )

		#define NEXT_TK_CHECK_TOKEN(_type, ...) do {\
				GET_NEXT_TOKEN;\
				CHECK_TOKEN(_type##__VA_ARGS__);\
			} while(0)

		#define NEXT_TK_CHECK_KEYWORD(_kw, ...) do {\
				GET_NEXT_TOKEN;\
...
/////////////////////////////////////////////////////////////////////////
	...
	#define NEXT_TK_CHECK_RULE(_rule) do {\
			GET_NEXT_TOKEN;\
			CHECK_RULE(_rule);\
		} while(0)
#endif
...
/////////////////////////////////////////////////////////////////////////
	...
	#define NEXT_TK_CHECK_VAR_ID do {\
			NEXT_TK_CHECK_TOKEN(ID);\
			SYNTAX(HAS_DOLLAR);\
		} while(0)

	#define NEXT_TK_CHECK_FUNC_ID do {\
			NEXT_TK_CHECK_TOKEN(ID);\
			SYNTAX(!HAS_DOLLAR);\
		} while(0)
#endif
